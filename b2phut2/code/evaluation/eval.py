...
def eval_model(dataset_dir, metadata_filename, model_filename,
    ...
    if dataset_split is "test":
        test_loader = prepare_dataloaders(dataset_split=dataset_split,
                                          ...
                                          sample_size=sample_size, num_worker=0, )
    elif dataset_split is "train":
        train_loader, valid_loader, test_loader = prepare_dataloaders(dataset_split=dataset_split,
                                                                      dataset_path=dataset_dir,
                                                                      metadata_filename=metadata_filename,
                                                                      batch_size=batch_size,
    ...
    model = ModularSVNHClassifier(cfg.MODEL,
                                  feature_transformation=ResNet34(current_hyper_params_dict["FEATURES_OUTPUT_SIZE"]),
    ...
    y_pred, y_true = performance_evaluator.evaluate(model, device, stats, mode="test")
