...
class Potterhsu(nn.Module):
    """
     This model is based on a project on github that attempted to solves the same problem.
    ...
    def __init__(self):
        ...
        hidden9 = nn.Sequential(nn.Linear(192 * 7 * 7, 3072), nn.ReLU())
        hidden10 = nn.Sequential(nn.Linear(3072, 3072), nn.ReLU())
    ...
    def forward(self, x):
        ...
        length_logits, digits_logits = (
            self._digit_length(x),
            [
                self._digit1(x),
                self._digit2(x),
                self._digit3(x),
